...
namespace OpenRA.Mods.Common.Traits
{
		...
		INotifyResourceClaimLost, INotifyIdle, INotifyBlockingMove
	{
		...
		Actor ClosestProc(Actor self, Actor ignore)
		{
			...
			var refs = (
				from r in self.World.ActorsWithTrait<IRefinery>()
		...
		public void UnblockRefinery(Actor self)
		{
			...
			{
				var deliveryLoc = lastproc.Location + lastproc.Trait<IRefinery>().DeliverOffset;
		...
		public IEnumerable<IOrderTargeter> Orders
		{
			get
			{
				yield return new EnterAlliedActorTargeter<IRefinery>("Deliver", 5,
					proc => IsAcceptableProcType(proc),
					proc => !IsEmpty && proc.Trait<IRefinery>().AllowDocking);
		...
		public void ResolveOrder(Actor self, Order order)
		{
			...
			else if (order.OrderString == "Deliver")
			{
				...
				var iao = order.TargetActor.TraitOrDefault<IRefinery>();
