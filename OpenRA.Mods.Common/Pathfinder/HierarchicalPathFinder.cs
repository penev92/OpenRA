...
namespace OpenRA.Mods.Common.Pathfinder
{
	...
	public sealed class HierarchicalPathFinder
	{
		...
		sealed class AbstractGraphWithInsertedEdges
		{
			...
			{
				var edges = abstractEdges[abstractCell];
				changedEdges.Add(localCell, edges
			...
			for (byte gridLayer = 0; gridLayer < customMovementLayers.Length; gridLayer++)
			{
				...
				while (accessibleCells.Count > 0)
				{
					...
					using (var search = GetLocalPathSearch(
						null, new[] { src }, src, null, null, BlockedByActor.None, false, grid, heuristicWeightPercentage: 100))
					{
			...
			PathFinderOverlay pathFinderOverlay)
		{
			...
			var validSources = new List<CPos>(sources.Count);
			...
			{
				var hasAnyAbstractPaths = false;
				foreach (var source in validSources)
				{
					reverseAbstractSearch.TargetPredicate = cell => cell == source;
					if (reverseAbstractSearch.ExpandToTarget())
						hasAnyAbstractPaths = true;
				}

				if (!hasAnyAbstractPaths)
					return PathFinder.NoPath;

				using (var fromSrc = GetLocalPathSearch(
					self, validSources, target, customCost, ignoreActor, check, laneBias, null,
					heuristic: Heuristic(reverseAbstractSearch), recorder: pathFinderOverlay?.RecordLocalEdges(self)))
			...
			PathFinderOverlay pathFinderOverlay)
		{
			...
			if ((target - source).LengthSquared < GridSize * GridSize * CloseGridDistance * CloseGridDistance && source.Layer == target.Layer)
			{
				...
				List<CPos> localPath;
				using (var search = GetLocalPathSearch(
					self, new[] { source }, target, customCost, ignoreActor, check, laneBias, gridToSearch, recorder: pathFinderOverlay?.RecordLocalEdges(self)))
				...
				fullGraph.GetConnections, locomotor, source, target, estimatedSearchSize, pathFinderOverlay?.RecordAbstractEdges(self)))
			{
					...
					fullGraph.GetConnections, locomotor, target, source, estimatedSearchSize, pathFinderOverlay?.RecordAbstractEdges(self)))
				{
					...
					using (var fromDest = GetLocalPathSearch(
						...
						heuristic: Heuristic(forwardAbstractSearch), recorder: pathFinderOverlay?.RecordLocalEdges(self), inReverse: true))
		...
		/// </summary>
		Func<CPos, int> Heuristic(PathSearch abstractSearch)
		{
			...
			return cell =>
			{
				...
				if (abstractCell.Layer != previousNode.Layer)
					previousNode = graph[previousNode].PreviousNode;

				var cost = graph[previousNode].CostSoFar + costEstimator(cell, previousNode);
				return cost;
			};
		}
