...
namespace OpenRA.Mods.Common.Traits.Buildings
{
	...
	public class BuildingInfo : ITraitInfo, IOccupySpaceInfo, UsesInit<LocationInit>
	{
		...
		public Actor FindBaseProvider (OpenRA.World world, Player p, CPos topLeft)
		{
			var center = world.Map.CenterOfCell(topLeft) + BuildingFootprintUtils.CenterOffset(world, this);
		...
		public bool IsCloseEnoughToBase (OpenRA.World world, Player p, string buildingName, CPos topLeft)
		{
			...
			var buildingTiles = BuildingFootprintUtils.Tiles(world.Map.Rules, buildingName, this, topLeft).ToList();
	...
	public class Building : IOccupySpace, INotifySold, INotifyTransform, ISync, ITechTreePrerequisite, INotifyCreated, INotifyAddedToWorld, INotifyRemovedFromWorld
	{
		...
		public Building(ActorInitializer init, BuildingInfo info)
		{
			...
			occupiedCells = BuildingFootprintUtils.UnpathableTiles( self.Info.Name, Info, TopLeft )
				.Select(c => Pair.New(c, SubCell.FullCell)).ToArray();

			CenterPosition = init.world.Map.CenterOfCell(topLeft) + BuildingFootprintUtils.CenterOffset(init.world, Info);
