...
namespace OpenRA.Network
{
	...
	public class WebSocketSession : IDisposable
	{
												  ...
												  + "Sec-WebSocket-Accept: " + Convert.ToBase64String(
													  SHA1.Create().ComputeHash(
														  Encoding.UTF8.GetBytes(
															  new Regex("Sec-WebSocket-Key: (.*)").Match(Encoding.UTF8.GetString(handshake)).Groups[1].Value.Trim() + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11"))) + Environment.NewLine
												  + Environment.NewLine);
		...
		static void SendMessage(TcpClient client, string payload, bool masking = false) => SendMessage(client, Encoding.UTF8.GetBytes(payload), false, masking);
		...
		static void SendMessage(TcpClient client, byte[] payload, int opcode, bool masking, byte[] mask)
		{
			...
			using (var packet = new MemoryStream())
			{
				...
				if (payload.LongLength <= 0b0_1111101)
				{
		...
		static byte[] ApplyMask(byte[] msg, byte[] mask)
		{
			var decoded = new byte[msg.Length];
